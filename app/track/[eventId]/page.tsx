    ...
    >
      ...
      {status === "connected" && (
        <div className="absolute inset-0 z-0 pointer-events-none flex items-center justify-center opacity-30">
          <div className="w-[800px] h-[800px] border border-emerald-500/20 rounded-full absolute" />
          <div className="w-[600px] h-[600px] border border-emerald-500/30 rounded-full absolute shadow-[0_0_50px_rgba(16,185,129,0.1)]" />
          <div className="w-[400px] h-[400px] border border-emerald-500/40 rounded-full absolute" />
          <div className="w-[200px] h-[200px] border border-emerald-500/60 rounded-full bg-emerald-500/5 backdrop-blur-3xl absolute animate-pulse" />
        </div>
      )}
      ...
      {/* ─── Top HUD ─── */}
      <div className="z-10 w-full max-w-sm flex justify-between items-center backdrop-blur-xl bg-black/40 border border-border/50 rounded-2xl p-3 px-5 shadow-2xl mb-8 mt-2">
        <div className="flex items-center gap-2">
          <RadioTower className="w-4 h-4 text-muted-foreground" />
          <span className="font-bold text-sm tracking-widest text-foreground dark:text-white uppercase">
            V-Sync
          </span>
        </div>
        {status === "connecting" && (
        ...
        {status === "connected" && (
          <Badge className="bg-emerald-500/20 text-emerald-400 border border-emerald-500/50 hover:bg-emerald-500/30 font-bold px-3 py-1 animate-pulse shadow-[0_0_15px_rgba(16,185,129,0.4)]">
            LIVE SECURE
          </Badge>
        )}
        {status === "out-of-bounds" && (
      ...
      >
        <div
          ...
          ${status === "connected" ? "bg-gradient-to-r from-transparent via-emerald-500 to-transparent" : ""}
        ...
        <CardContent className="pt-8 text-center space-y-6 pb-8">
          ...
          {status === "connected" && (
            <div className="space-y-6">
              <div className="relative w-24 h-24 mx-auto">
                ...
                <div className="absolute inset-2 bg-emerald-500/20 rounded-full border border-emerald-500/50 shadow-[0_0_20px_rgba(16,185,129,0.4)] flex items-center justify-center z-10 backdrop-blur-sm">
                  <SignalHigh className="text-emerald-400 w-8 h-8" />
                </div>
              </div>
              ...
              {coords && (
                <div className="inline-flex items-center gap-2 bg-background border border-border px-4 py-2 rounded-lg font-mono text-xs text-emerald-400 shadow-inner">
              ...
              <div className="pt-2">
                <Button
                  ...
                  className="w-full h-14 rounded-xl font-bold uppercase tracking-wider bg-rose-600 hover:bg-rose-500 text-foreground dark:text-white flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(225,29,72,0.4)] hover:shadow-[0_0_30px_rgba(225,29,72,0.6)] transition-all"
                >
          ...
          {(status === "error" || status === "connecting") && (
            <div className="space-y-6 py-4">
              <div className="w-16 h-16 border-4 border-border border-t-slate-500 rounded-full animate-spin mx-auto" />
              <div>
                <h2 className="text-lg font-bold text-muted-foreground dark:text-slate-300 uppercase">
                  {status === "error"
                    ? "GPS Unavailable"
      ...
      <Dialog open={showSosDialog} onOpenChange={setShowSosDialog}>
        <DialogContent className="bg-slate-900 border-rose-500/30 text-foreground max-w-sm">
          <DialogHeader>
          ...
          <div className="flex gap-3 pt-4">
            <Button
              variant="outline"
              className="flex-1 border-slate-700 text-muted-foreground dark:text-slate-300"
              onClick={() => setShowSosDialog(false)}
            >
              Cancel
            </Button>
            <Button
              className="flex-1 bg-rose-600 hover:bg-rose-500 text-foreground dark:text-white font-bold shadow-[0_0_15px_rgba(225,29,72,0.4)]"
              onClick={handleSOS}
            >
              Send SOS Now
            </Button>
      ...
      {/* ─── Footer ─── */}
      <div className="absolute bottom-6 text-emerald-500/40 font-mono text-[10px] tracking-widest text-center z-10">
        SYSTEM // VENECK
        <br />
        {userId || "AWAITING ID"}
      </div>
